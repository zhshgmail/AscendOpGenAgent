...
class Model(nn.Module):
    """
    Simple model that performs element-wise logical OR with broadcasting support.
    torch.logical_or(input, other) -> Tensor
    """
    def __init__(self):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Applies element-wise logical OR to the input tensors with broadcasting support.
        ...
        Returns:
            torch.Tensor: Output bool tensor of the logical OR result.
        """
        return torch.logical_or(x, y)


def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), "4_LogicalOr.json")
